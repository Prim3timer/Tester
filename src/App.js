...
function App() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        "https://ukuta.onrender.com/results",
        result,
      );
      // const response = []
  ...
  const getResult = async () => {
    ...
    if (report) {
      ...
      setPresent(true);
      const currentResult = report.data.questions.find(
        (assess) => assess.ade === id,
      );
  ...
  let allComponents = reveal ? (
    ...
    <h2 id="submitting">Submitting Work...</h2>
  ) : isDone ? (
    <CheckResult
      getResult={getResult}
      setPresent={setPresent}
    ...
    <h2 id="getting">Getting Result...</h2>
  ) : present ? (
    <ResultSheet
      showSheet={showSheet}
      final={final}
  ...
  return (
    <div id="app">
      <div id="header">
        ...
        {started && (
          <h5 id={clock < 20 ? "winding-clock" : "clock"}>
            {clock < 10
              ? `0:0${clock % 60}`
              : clock < 60
                ? `0:${clock % 60}`
                : clock % 60 >= 10
                  ? `${Math.floor(clock / 60)}:${clock % 60}`
                  : clock < 10
                    ? 0`${clock % 60}`
                    : `${Math.floor(clock / 60)}:0${clock % 60}`}
          </h5>
        )}
        {candidate === "Dike" || (allResults && started) ? (
          <button className="admin" onClick={showResults}>
            View Results
          </button>
        ) : (
          ""
      ...
      {/* <General/> */}
      <span id="copy">
        <small>&copy; {year} Amalu Productions.</small>
      </span>
    </div>
  );
}
