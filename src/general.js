...
const General = ({
  ...
  const fetchResult = async () => {
    try {
      const response = await axios.get("https://ukuta.onrender.com/results");
  ...
  const Result = (
    ...
    const remover = async (id) => {
      ...
      setCancel(true);
      await axios.delete(
        `https://mawuhi-back.onrender.com/results/delete/${id}`,
      );
    ...
    const getTrans = async () => {
      // e.preventDefault()
      try {
        const graw = await axios.get(
          "https://mawuhi-back.onrender.com/results",
        );
        ...
        if (graw.data.questions.length > 0) {
          ...
          const filterate = graw.data.questions.filter((inner) =>
            inner.candidate.toLowerCase().includes(search.toLowerCase()),
          );
    ...
    return (
      <div>
        <table className="general-table">
          <tbody>
            <tr style={{ backgroundColor: "#4DE0D0" }}>
              <th>NAME</th>
              <th>UNIQUE ID</th>
              <th>SCORE (%)</th>
              <th>DATE</th>
            ...
            {results &&
              results.map((result, index) => {
                return (
                  <tr
                    style={{
                      backgroundColor: index % 2 === 0 ? "white" : "#AFEEEE",
                    }}
                  >
                    <td onClick={() => showOne(result.ade)}>
                      {result.candidate}
                    </td>
                    <td onClick={() => showOne(result.ade)}>{result.ade}</td>
                    <td
                      style={{
                        fontWeight: "bold",
                      }}
        ...
        <section id="trans-verify-section">
          {cancel ? (
            <div className="general-warning-container">
              {" "}
              <h3
                id="verify-header"
                style={{
              ...
              >
                Delete "{reportCard.candidate}'s result" ?
              </h3>
              <article className="general-warning">
                <button
                  style={{
  ...
  return (
    <div className="general-result-container">
      <article className="general-result-control">
        <button onClick={mainPage}>Back to Test</button>
        <button onClick={changePages}>Sheets</button>
      </article>
      <form>
        <input
          ref={resultRef}
      ...
      <h3 style={{ color: "white" }}>CANDIDATE RESULTS ({results.length})</h3>
      {/* { !results  ? <h3>Loading...</h3> : <Result/>}<br/> */}

      {<h3>Loading...</h3> && allResults ? (
